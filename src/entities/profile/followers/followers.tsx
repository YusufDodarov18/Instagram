...
function Followers({open,onClose}:{open:boolean,onClose:()=>void}) {
    const {followersLoading,myFollowing,myFollowers,addFollowing,unFollowing}=useProfile()
    ...
    useEffect(()=>{
        if(open&&myFollowers){
            const followingId=new Set(myFollowing?.map((f)=>f.userShortInfo?.userId))
            setFollowers(myFollowers.map((follower)=>({
                ...follower,
                isFollowing:followingId.has(follower.userShortInfo?.userId)
            })))
        }
    },[open,myFollowers,myFollowing])

    const toggleFollow=async(userId:string,isCurrentlyFollowing:boolean)=>{
        if(!decode)return
        setFollowers((prev)=>prev.map(user=>user.userShortInfo.userId===userId?{...user,isFollowing:!isCurrentlyFollowing}:user))
    ...
    const filteredFollowers=followers.filter((elem)=>elem.userShortInfo.userName.toLowerCase().includes(search.toLowerCase().trim())||elem.userShortInfo.fullname.toLowerCase().includes(search.toLowerCase().trim()))
            ...
            <form onSubmit={(e) => e.preventDefault()}>
                <DialogContent className="sm:max-w-md max-h-[70vh] overflow-hidden overflow-y-auto [&>button]:hidden dark:bg-[#1b1b1b] dark:text-white bg-white text-[black]">
                    <DialogHeader className="border-b pb-3 relative">
                        <DialogTitle className="text-center w-full">{t("yourFollowing")}</DialogTitle>
                        <span className="absolute right-[0] top-[-21px] text-[36px] cursor-pointer" onClick={onClose}>×</span>
                    </DialogHeader>
                    ...
                    <section>
                        <Box sx={{display:'flex',justifyContent:'center',gap:3,flexDirection:"column"}}>
                            {followersLoading?(
