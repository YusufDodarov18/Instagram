...
export default function Following({open,onClose}:{open:boolean,onClose:()=>void}) {
   ...
   useEffect(() =>{
    ...
    const toggleFollow= async(userId:string ,isCurrentlyFollowing:boolean) =>{
        if(decode){
            setFollowing((prev)=>prev.map(user=>user.userShortInfo.userId==userId?{...user,isFollowing:!isCurrentlyFollowing}:user))
            try {
    ...
    const filteredFollowing = following.filter(elem =>
        elem.userShortInfo.userName.toLowerCase().includes(search.toLowerCase().trim()) ||
        elem.userShortInfo.fullname.toLowerCase().includes(search.toLowerCase().trim())
    )
