...
function page() {
  ...
  const genderOptions = [
    ...
    { value: "prefer_not_to_say", label: t("setting.account.Prefer not to say") }
  ];

  useEffect(() =>{ getMyProfile()},[getMyProfile]);
  ...
  async function editProfile() {
    ...
    const isChanged=newProfile.about !==oldProfile.about ||newProfile.gender !== oldProfile.gender ||newProfile.customGender !== oldProfile.customGender;
    if (!isChanged) {
      return;
    }
  ...
  return (
    <>
      <div className="h-[100%] flex items-center justify-center py-0 md:py-20">
        <div className="flex flex-col gap-12 px-6 py-5">
          ...
          <div className="w-[100%] flex justify-between items-center px-9 py-6 rounded-2xl bg-[#F3F5F7] dark:bg-[#262626]">
            ...
            <div className="cursor-pointer">
              <Button className="bg-[#4A5DF9] hover:bg-[#4a56f9] text-white font-bold w-[120px] h-[36px]" onClick={() => changePhoto(myProfile?.image || "")}>{t("setting.account.Change photo")}</Button>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1 relative">
            ...
            <div className="relative">
              <textarea
                className="w-[100%] text-xl px-5 py-2 resize-none border border-gray-300 dark:border-white rounded-2xl"
                name="about" value={aboutText}
                onChange={handleChange}
                maxLength={maxLenght}
              />
              <p className="absolute bottom-2 right-3 text-sm text-gray-500 dark:text-gray-300">{aboutText.length}/{maxLenght}</p>
            </div>
          </div>
          <div className="flex flex-col gap-2">
            ...
            <div className="rounded-2xl border-1 flex items-center py-5 px-5 gap-10 justify-between">
              <div className="">
                ...
                <p className="text-[#6A717A] dark:text-[#A8A8A8] text-wrap">
                  {t("setting.account.Choose whether people can see similar account suggestions on your profile, and whether your account can be suggested on other profiles.")}
                </p>
              </div>
          ...
          <div className="flex justify-end">
            <Button className="bg-[#4A5DF9] hover:bg-[#4A5DF9] text-white font-black w-70 h-15 cursor-pointer" onClick={editProfile}>
      ...
      <Dialog open={modalImage} onOpenChange={() => setModalImage(false)}>
        <DialogContent className="p-0 m-0 py-4 bg-white dark:bg-[#262626] !max-w-[500px] w-full [&>button]:hidden rounded-lg overflow-hidden">
          ...
          <div className="flex flex-col divide-y divide-gray-300 dark:divide-gray-700 text-lg text-center">
            <div className="py-3 text-blue-700 font-black cursor-pointer" onClick={() => inputFile.current?.click()}>
              {t("setting.account.Upload photo")}
            </div>
            <div className="py-3 text-red-600 cursor-pointer font-black "
              onClick={async () => {
            ...
            <div className="py-3 cursor-pointer" onClick={() => setModalImage(false)}>
              {t("Cancel")}
            </div>
          </div>
        </DialogContent>
