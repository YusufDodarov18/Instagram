...
function Layout({children}:{children:React.ReactNode}) {
     const {chats,deleteChat,getChats, getChatById,getLastMessage,lastMessage,createChat,chatById,datas,searchUsers}=useChats()
       ...
       const router=useRouter()
        ...
        return (
              <>
                                                                       ...
                                                                       <div key={chat.chatId} className={`flex group  w-[100%] items-center gap-3 px-5 py-2 transition-colors cursor-pointer ${Number(activeChatId) === chat.chatId ? "bg-accent" : "hover:bg-accent/50"}`} key={chat.chatId}>
                                                                           ...
                                                                           <div className='flex-1 min-w-0 text-left'>
                                                                               ...
                                                                               <div className='flex items-center gap-1'>
                                                                                   <span className={`text-sm truncate font-semibold`}>{chat?.lastMessage||t("no message")}</span>  
                                                                               </div>
                                                                           </div>
                   ...
                   {menu &&(
                          <div className='fixed z-[9999]' style={{ top: menu.y,left: menu.x}}>
                                 <div className='flex justify-center px-4 py-4 flex-col font-bold items-center rounded-3xl gap-5 bg-[#f7f7f7] dark:bg-[#242323] w-[220px]'>
                                          ...
                                          <div className='flex justify-between gap-1  items-center text-red-500 cursor-pointer'  onClick={async()=>{await deleteChat(selectedChatId)}}>
                                                 <p>{t("delete")}</p>
